...
def build(submittals_path, qtycon_path, mobilization_path):
    ...
    location_lookup_sheet = xl.parse("Location")

    db = db.rename(columns={"Tile": "Zone_unused_raw", "Building Levels": "Level"})
    db["Date Created"] = pd.to_datetime(db["Date Created"])

    # ---- Location / Area ----
    # IMPORTANT: the column literally named "Location/Area" in the submittal
    # log is NOT usable (it holds a duplicate timestamp, not location text —
    # see README). The real Location/Area category for each document is
    # encoded as the 6th "-"-separated segment of the Document No (e.g.
    # "QRUT171-USC-C03221-WIR-CIV-CP-CB4ZZZ-CB0315" -> "CP"), and the human-
    # readable label for each code lives in the separate "Location" lookup
    # sheet (e.g. "CP" -> "CP - Car Park"). We reconstruct it here.
    location_code_lookup = {}
    for val in location_lookup_sheet["Location/Area"].dropna():
        code = str(val).split(" - ")[0].strip()
        location_code_lookup[code] = val
    doc_location_code = db["Document No"].str.split("-").str[5]
    db["Zone"] = doc_location_code.map(location_code_lookup).fillna(doc_location_code)
